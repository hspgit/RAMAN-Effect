...
| **CNN-Transformer Fusion**| Fragment extraction (CNN) + Global Attention | **60.87%** | Successfully beat the pure Transformer by using a CNN to extract local peaks first, but the Transformer backend still lacked the data variety needed to fully conquer the spatial relationships. |
| **FusionNet + MixUp**| MixUp Augmentation (Alpha=0.2) | **55.80%** | We applied MixUp to prevent the Transformer from overfitting. Unfortunately, mathematically blending Raman peaks on-the-fly degraded the physical meaning of the spectra too much, causing the model to learn artificial shapes rather than true chemical peaks. |
| **1D Transformer** | Patchification (100-pt patches), 3 Layers, 4 Heads | **54.73%** | Transformers lack spatial "inductive bias". Without a massive dataset, it severely underfitted and failed to learn the basic shapes of the chemical peaks. |
